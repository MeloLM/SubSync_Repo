...
export function LoginForm() {
  ...
  function handleGoogle() {
    startOAuth(async () => {
      ...
      // In caso di successo la Server Action reindirizza a Google: nessun ritorno.
      if (res?.error) {
  ...
  const busy = isPending || isOAuthPending;
  ...
  return (
    <div className="space-y-4">
      <button
        ...
        disabled={busy}
        className="inline-flex w-full items-center justify-center gap-2 rounded-lg border border-zinc-700 bg-zinc-100 px-4 py-2.5 text-sm font-medium text-zinc-900 transition-colors hover:bg-white disabled:opacity-60"
      >
        {isOAuthPending ? (
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        >
          ...
          {isPending ? "Attendere..." : isLogin ? "Accedi" : "Crea account"}
        </button>
